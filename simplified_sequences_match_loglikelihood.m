function loglikelihood = simplified_sequences_match_loglikelihood(Si, So, lambda, return_loglikelihood_sequences)
% loglikelihood = sequences_match_loglikelihood(Si, So, lambda, return_loglikelihood_sequences)
%
...
%
  ...
  for k = 1:length(Si)
    L_ti_to = log(erlang_pdf(So(k) - Si(k), 3, lambda));
    if return_loglikelihood_sequences
