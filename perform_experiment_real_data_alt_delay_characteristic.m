...
# database_id = 'exp02_nym-binaries-1.0.2_static-http-download_no-client-cover-traffic_filtered-to-start-end-main';
  ...
  average_mixnet_delay = 150e-3 + 4*51.103e-3;
  time_units = 1e-9;
  ...
  client_block_size = ceil(test_set_samples_count/client_subsets_count);
  client_indexes = (1:client_block_size) + (client_subset_index - 1)*client_block_size;
  if client_indexes(end) > test_set_samples_count
    client_indexes = client_indexes(1):test_set_samples_count;
  endif
  clients_count = length(client_indexes);
  server_indexes = 1:test_set_samples_count;

  score_matrix_test = zeros(clients_count, test_set_samples_count, length(lengths_vector) + 1);
  data_mean_delay_test = zeros(clients_count, test_set_samples_count);
  data_min_delay_test = zeros(clients_count, test_set_samples_count);
  ack_mean_delay_test = zeros(clients_count, test_set_samples_count);
  ack_min_delay_test = zeros(clients_count, test_set_samples_count);

  for client_index = client_indexes
    max_false_attempt_to_this_client_score = -Inf;
    for server_index = server_indexes
      data_transmission_times = sort([data_initiator_to_gateway_test{client_index}; data_responder_to_gateway_test{server_index}], 'ascend');
      data_reception_times = sort([data_initiator_from_gateway_test{client_index}; data_responder_from_gateway_test{server_index}], 'ascend');
      ack_transmission_times = data_reception_times;
      ack_reception_times = sort([ack_initiator_from_gateway_test{client_index}; ack_responder_from_gateway_test{server_index}], 'ascend');
      [aligned_data_transmission_times, aligned_data_reception_times] = align_sequences(data_transmission_times, data_reception_times, average_mixnet_delay);
      [aligned_ack_transmission_times, aligned_ack_reception_times] = align_sequences(ack_transmission_times, ack_reception_times, average_mixnet_delay);
      data_mean_delay_test(mod(client_index - 1, client_block_size) + 1, server_index) = mean(aligned_data_reception_times - aligned_data_transmission_times);
      ack_mean_delay_test(mod(client_index - 1, client_block_size) + 1, server_index) = mean(aligned_ack_reception_times - aligned_ack_transmission_times);
      data_min_delay_test(mod(client_index - 1, client_block_size) + 1, server_index) = min(aligned_data_reception_times - aligned_data_transmission_times);
      ack_min_delay_test(mod(client_index - 1, client_block_size) + 1, server_index) = min(aligned_ack_reception_times - aligned_ack_transmission_times);
      % fprintf(log_handler, '\rClient %4i, Server %4i.\tMin data delay = %7.2f ms;\tMin ack delay: %7.2f ms;\tMean data delay = %7.2f ms;\tMean ack delay: %7.2f ms.\t', client_index, server_index, 1e3*data_min_delay_test(mod(client_index - 1, client_block_size) + 1, server_index), 1e3*ack_min_delay_test(mod(client_index - 1, client_block_size) + 1, server_index), 1e3*data_mean_delay_test(mod(client_index - 1, client_block_size) + 1, server_index), 1e3*ack_mean_delay_test(mod(client_index - 1, client_block_size) + 1, server_index));
      fprintf(1, '\rClient %4i, Server %4i.\tMin data delay = %7.2f ms;\tMin ack delay: %7.2f ms;\tMean data delay = %7.2f ms;\tMean ack delay: %7.2f ms.\t', client_index, server_index, 1e3*data_min_delay_test(mod(client_index - 1, client_block_size) + 1, server_index), 1e3*ack_min_delay_test(mod(client_index - 1, client_block_size) + 1, server_index), 1e3*data_mean_delay_test(mod(client_index - 1, client_block_size) + 1, server_index), 1e3*ack_mean_delay_test(mod(client_index - 1, client_block_size) + 1, server_index));
      assert(ack_mean_delay_test(mod(client_index - 1, client_block_size) + 1, server_index) >= average_mixnet_delay)
      assert(data_mean_delay_test(mod(client_index - 1, client_block_size) + 1, server_index) >= average_mixnet_delay)
      min_length = min([length(aligned_data_transmission_times); length(aligned_ack_transmission_times)]);
      score_vector = ...
      gaussian_delay_sequences_match_loglikelihood(aligned_data_transmission_times(1:min_length), aligned_data_reception_times(1:min_length), average_delay, std_dev_delay, true) + ...
      gaussian_delay_sequences_match_loglikelihood(aligned_ack_transmission_times(1:min_length), aligned_ack_reception_times(1:min_length), average_delay, std_dev_delay, true);
	...
	score_matrix_test(mod(client_index - 1, client_block_size) + 1, server_index, k) = mean(score_vector(1:lengths_vector(k)));
      ...
      if client_index == server_index
	% fprintf(log_handler, ['LLK(%4i) = ' char(27) '[1;49;92m%.2f' char(27) '[0m. \t'], min_length, score_matrix_test(mod(client_index - 1, client_block_size) + 1, server_index, end)); fflush(1);
	...
	% fprintf(log_handler, ['LLK(%4i) = ' char(27) '[0;49;31m%.2f' char(27) '[0m. \t'], min_length, score_matrix_test(mod(client_index - 1, client_block_size) + 1, server_index, end)); fflush(1);
	fprintf(1, ['LLK(%4i) = ' char(27) '[0;49;31m%.2f' char(27) '[0m. \t'], min_length, score_matrix_test(mod(client_index - 1, client_block_size) + 1, server_index, end)); fflush(1);
      ...
      % fprintf(log_handler, 'Progress: %.6f%%', 100.0*double(mod(client_index - 1, client_block_size)*test_set_samples_count + server_index)/double(test_set_samples_count*client_block_size)); fflush(1);
